...
const SITE_DATA_CACHE_VERSION = "site-lite-category-fix-4-20260621";
  ...
  return `
    <div class="gender-filter${options.compact ? " gender-filter--compact" : ""}" data-gender-filter>
      ${visibleOptions.map((option) => {
        ...
        const label = `${escapeHtml(option.label)} <em>${counts[option.value] || 0}</em>`;
        return asLinks
...
function initPwaShell() {
  ...
  if ("caches" in window) {
    caches.keys().then((keys) => {
      keys
        .filter((key) => key.startsWith("threon-") && key !== "threon-category-fix-4-v20260621")
        .forEach((key) => caches.delete(key));
    }).catch(() => {});
  }
  if ("serviceWorker" in navigator && location.hostname !== "") {
    navigator.serviceWorker.register("service-worker.js?v=threon-category-fix-4-20260621").catch(() => {});
  }
}
...
async function initCollection() {
  ...
  try {
    ...
    const baseProducts = activeCategory
      ...
      : data.products;
    const products = activeGender === "all" ? baseProducts : baseProducts.filter((product) => productHasGender(product, activeGender));
