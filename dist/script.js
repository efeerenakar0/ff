...
const SITE_DATA_CACHE_VERSION = "site-lite-cloudflare-split-20260625";
...
const CATEGORY_TREE = {
  ...
  "Erkek": ["Yeni Gelenler", "Üst Giyim", "Alt Giyim", "Dış Giyim", "Takımlar", "İç Giyim", "Plaj Giyim", "Aksesuar", "Unisex"],
  "Takımlar": ["Eşofman Takımı", "Alt Üst Takım", "Takım Elbise", "Şort Takım", "Kapsül Set"],
  "Üst Giyim": ["Kapüşonlu", "T-Shirt", "Gömlek", "Yelek", "Sweatshirt", "Triko | Kazak", "Elbise"],
  "Alt Giyim": ["Pantolon", "Kargo Pantolon", "Jeans", "Şort", "Eşofman Altı", "Etek", "Çorap", "Terlik", "Ayakkabı"],
  ...
  "Aksesuar": ["Çanta", "Cüzdan", "Şapka", "Parfüm", "Aksesuar", "Kemer", "Takı", "Gözlük"]
};

const CATEGORY_IMAGES = {
  "Kadın": "assets/threon-fashion-hero.png",
  "Erkek": "assets/product-bomber.png",
  ...
  "Plaj Giyim": "assets/product-tee.png",
  "İç Giyim": "assets/product-tee.png",
...
const CATEGORY_COPY = {
  ...
  "Erkek": "Erkek seçkisi, unisex kapsül parçalar ve günlük premium siluetler.",
...
const CATEGORY_ROUTE_ALIASES = {
  ...
  erkek: { category: "Erkek", subcategory: "" },
  erkekler: { category: "Erkek", subcategory: "" },
  men: { category: "Erkek", subcategory: "" },
  mens: { category: "Erkek", subcategory: "" },
  ...
  "erkek-yeni-gelenler": { category: "Erkek", subcategory: "Yeni Gelenler" },
  "erkek-ust-giyim": { category: "Erkek", subcategory: "Üst Giyim" },
  "erkek-alt-giyim": { category: "Erkek", subcategory: "Alt Giyim" },
  "erkek-dis-giyim": { category: "Erkek", subcategory: "Dış Giyim" },
  "erkek-takimlar": { category: "Erkek", subcategory: "Takımlar" },
  "erkek-ic-giyim": { category: "Erkek", subcategory: "İç Giyim" },
  "erkek-plaj-giyim": { category: "Erkek", subcategory: "Plaj Giyim" },
  "erkek-aksesuar": { category: "Erkek", subcategory: "Aksesuar" },
  "erkek-unisex": { category: "Erkek", subcategory: "Unisex" },
...
const DEFAULT_TICKER_ITEMS = ["YENİ GELENLER", "OVERSIZE KALIP", "STÜDYO SEÇKİSİ", "1500 TL+ ÜCRETSİZ KARGO", "THREE GIFT CARD"];
...
function inferInnerwearSubcategory(product = {}) {
  ...
  if (/boxer/.test(text) && /erkek|bay\b|men\b|mens\b/.test(text)) return "Boxer";
  if (/külot|kulot|boyshort|brazilian|hipster|slip|string|tanga/.test(text)) return "Külot";
  if (/boxer/.test(text)) return "Boxer";
...
function productHasGender(product = {}, gender = "") {
  ...
  if (gender === "Kadın") return sections.includes("Kadın") || sections.includes("Unisex");
  if (gender === "Erkek") return sections.includes("Erkek") || sections.includes("Unisex");
  if (gender === "Unisex") return sections.includes("Unisex");
  return sections.includes(gender);
}

function genderFromValue(value = "") {
  const normalized = slugify(value);
  if (!normalized || normalized === "all" || normalized === "tum") return "all";
  if (["kadin", "women", "womens"].includes(normalized)) return "Kadın";
  if (["erkek", "men", "mens"].includes(normalized)) return "Erkek";
...
function productMenSubcategories(product = {}) {
  const sections = new Set();
  const mainCategory = productMainCategory(product);
  const genderSections = normalizedGenderSections(product);
  const text = [product.name, product.slug, product.summary, product.description, ...(product.features || []), ...(product.tags || [])]
  ...
  if (mainCategory && mainCategory !== "Erkek") sections.add(mainCategory);
  if (genderSections.includes("Unisex") || /\bunisex\b/.test(text)) sections.add("Unisex");
  if (product.featured) sections.add("Yeni Gelenler");

  return [...sections].filter((section) => CATEGORY_TREE["Erkek"].includes(section));
}

function productIsInMenSection(product = {}) {
  const mainCategory = productMainCategory(product);
  const genderSections = normalizedGenderSections(product);
  return mainCategory === "Erkek" || genderSections.includes("Erkek") || genderSections.includes("Unisex");
}
...
function categoryUrl(category = "", subcategory = "") {
  ...
  if (category === "Erkek" && subcategory && subcategory !== "all") {
    return `/erkek-${slugify(subcategory)}`;
  }
...
function productMatchesCategory(product, category = "all", subcategory = "all") {
  ...
  if (category === "Erkek") {
    const menMatch = productIsInMenSection(product);
    const subcategoryMatch = subcategory === "all" || productMenSubcategories(product).includes(subcategory);
    return menMatch && subcategoryMatch;
  }
  const mainCategory = productMainCategory(product);
...
function genderFilterCounts(products = []) {
  return GENDER_FILTER_OPTIONS.reduce((counts, option) => {
    counts[option.value] =
      option.value === "all" ? products.length : products.filter((product) => productHasGender(product, option.value)).length;
    return counts;
  }, {});
}

function allowedGenderValuesForCategory(category = "all") {
  if (category === "Kadın") return ["all", "Kadın", "Unisex"];
  if (category === "Erkek") return ["all", "Erkek", "Unisex"];
  return GENDER_FILTER_OPTIONS.map((option) => option.value);
}

function normalizeAllowedGender(gender = "all", category = "all") {
  const clean = genderFromValue(gender);
  return allowedGenderValuesForCategory(category).includes(clean) ? clean : "all";
}

function genderUrl(baseUrl = "", gender = "all") {
  const [pathname, query = ""] = String(baseUrl || "/koleksiyon").split("?");
  const params = new URLSearchParams(query);
  if (!gender || gender === "all") params.delete("gender");
  else params.set("gender", gender);
  const nextQuery = params.toString();
  return `${pathname}${nextQuery ? `?${nextQuery}` : ""}`;
}

function renderGenderFilterMarkup(activeGender = "all", products = [], options = {}) {
  const counts = genderFilterCounts(products);
  const asLinks = Boolean(options.asLinks);
  const baseUrl = options.baseUrl || "/koleksiyon";
  const allowedValues = Array.isArray(options.allowedValues) && options.allowedValues.length ? options.allowedValues : GENDER_FILTER_OPTIONS.map((option) => option.value);
  const visibleOptions = GENDER_FILTER_OPTIONS.filter((option) => allowedValues.includes(option.value));
  return `
    <div class="gender-filter${options.compact ? " gender-filter--compact" : ""}" data-gender-filter>
      ${visibleOptions.map((option) => {
        const active = option.value === activeGender ? " is-active" : "";
        const disabled = counts[option.value] === 0 ? " is-disabled" : "";
        const label = escapeHtml(option.label);
        return asLinks
          ? `<a class="gender-filter__item${active}${disabled}" href="${genderUrl(baseUrl, option.value)}">${label}</a>`
          : `<button class="gender-filter__item${active}${disabled}" type="button" data-filter-gender="${escapeHtml(option.value)}">${label}</button>`;
      }).join("")}
    </div>
  `;
}
...
function appendDataVersion(url = "") {
  if (!url) return url;
  return `${url}${url.includes("?") ? "&" : "?"}v=${encodeURIComponent(SITE_DATA_CACHE_VERSION)}`;
}

function chunkEntriesFromPayload(payload = {}) {
  const chunks = Array.isArray(payload.productChunks)
    ? payload.productChunks
    : Array.isArray(payload.productsChunks)
      ? payload.productsChunks
      : [];
  return chunks
    .map((chunk) => (typeof chunk === "string" ? { file: chunk } : chunk))
    .filter((chunk) => chunk && chunk.file)
    .map((chunk) => ({ ...chunk, file: dataAssetUrl(chunk.file) }));
}

async function fetchProductsIndex(payload = {}) {
  const inlineChunks = chunkEntriesFromPayload(payload);
  if (inlineChunks.length) return { ...payload, productChunks: inlineChunks };
  const indexUrl = dataAssetUrl(payload.productsIndex || payload.productIndex || "data/products-index.json");
  const index = await fetchJson(appendDataVersion(indexUrl));
  return { ...payload, ...index, productChunks: chunkEntriesFromPayload(index) };
}

async function hydrateSitePayload(payload = {}) {
  if (Array.isArray(payload.products) && payload.products.length) {
    return publicSiteData(payload);
  }
  const indexed = await fetchProductsIndex(payload);
  const products = [];
  for (const chunk of chunkEntriesFromPayload(indexed)) {
    const chunkData = await fetchJson(appendDataVersion(chunk.file));
    const chunkProducts = Array.isArray(chunkData) ? chunkData : Array.isArray(chunkData.products) ? chunkData.products : [];
    products.push(...chunkProducts);
  }
  return publicSiteData({
    settings: indexed.settings || payload.settings || {},
    products
  });
}
...
function productFeatureItems(product = {}) {
  const importNoise = /kaynak|aktarıldı|aktarildi|stoklu varyant|varyant ve stok|ürün açıklamasından|urun aciklamasindan|yetkili panel/i;
...
function productIsLofibuy(product = {}) {
  return [product.id, product.slug, product.sku, product.source, product.vendor, product.brand, product.image]
    .filter(Boolean)
    .some((value) => String(value).toLowerCase().includes("lofibuy") || String(value).toLowerCase().includes("lofi"));
}

function collectionProductsForDisplay(products = [], category = "", subcategory = "") {
  const items = [...products];
  if (category === "Takımlar" && subcategory === "Alt Üst Takım") {
    return items.sort((a, b) => Number(productIsLofibuy(b)) - Number(productIsLofibuy(a)) || productMerchandisingScore(b) - productMerchandisingScore(a));
  }
  return items;
}
...
function publicProductSummary(product = {}) {
  ...
  return {
    ...
    material: productMaterialText(product),
    care: productCareText(product),
    features: productFeatureItems(product),
    modelInfo: cleanDetailText(product.modelInfo, 160),
    shippingNote: cleanDetailText(product.shippingNote, 160),
    specs,
...
async function loadStaticSiteData() {
  const data = await fetchJson(appendDataVersion("data/site-public.json"));
  const hydrated = await hydrateSitePayload(data);
  if (!Array.isArray(hydrated.products)) {
...
function fetchSite() {
  if (!state.sitePromise) {
    ...
    if (cached) {
      ...
      state.sitePromise = Promise.resolve(cached);
      fetchJson("/api/site")
        .then((data) => hydrateSitePayload(data))
        .then((data) => {
    ...
    state.sitePromise = fetchJson("/api/site")
      ...
      .catch(async () => {
        const fallback = await loadFallbackSiteData();
        state.site = fallback;
        renderCart();
        renderWishlist();
        renderCompareDrawer();
...
function parseTickerItems(value) {
  ...
  return items.length ? items : [...DEFAULT_TICKER_ITEMS];
}

function renderTicker(settings = state.site?.settings || {}) {
  const ticker = qs(".premium-drop-ticker div");
  if (!ticker) return;
  const items = parseTickerItems(settings.tickerItems);
  ticker.innerHTML = [...items, ...items].map((item) => `<span>${escapeHtml(item)}</span>`).join("");
}
...
function productsForDynamicShelf(shelf = {}, products = []) {
  ...
  if (normalizedShelf.mode === "manual") {
    ...
    selected = collectionProductsForDisplay(selected, normalizedShelf.category, normalizedShelf.subcategory);
...
function recommendationScore(source = {}, candidate = {}) {
  ...
  if (sourceCategory && sourceCategory === candidateCategory) score += 140;
  if (sourceSubcategory && sourceSubcategory === candidateSubcategory) score += 120;
...
function visualBlockUrl(block = {}) {
  const product = productByVisualKey(block.productSlug);
    ...
    <article class="product-card${hasHoverImage ? " product-card--has-hover" : ""}" data-product-card="${escapeHtml(product.id)}">
      <div class="product-card__media product-card__media--${escapeHtml(imageRatio)}" style="${escapeHtml(mediaStyle)}">
        <a href="${productUrl(product)}" aria-label="${escapeHtml(product.name)} detay">
          ...
          ${
            hoverImage
              ? `<img class="product-card__image product-card__image--hover" src="${escapeHtml(hoverImage)}" alt="${escapeHtml(product.name)} alternatif görsel" loading="lazy" decoding="async" style="${escapeHtml(imageStyleText)}" />`
              : ""
          }
        </a>
    ...
    `
      <section class="premium-footer" data-premium-footer>
        ...
        <div class="premium-footer__contact">
          <h3>THREON Club</h3>
          <p>Daha hızlı ödeme, sipariş geçmişi ve erken kapsül duyuruları için hesap oluştur.</p>
...
function initPwaShell() {
  ...
  if ("caches" in window) {
    caches.keys().then((keys) => {
      keys
        .filter((key) => key.startsWith("threon-") && key !== "threon-admin-policy-fix-v20260621b")
        .forEach((key) => caches.delete(key));
    }).catch(() => {});
  }
  if ("serviceWorker" in navigator && location.hostname !== "") {
    navigator.serviceWorker.register("service-worker.js?v=threon-admin-policy-fix-20260621b").catch(() => {});
  }
}
...
function usableHomeProduct(product = {}) {
  const image = String(product.image || "");
  const mainCategory = productMainCategory(product);
  return (
    product.status !== "draft" &&
    image &&
    image !== "assets/threon-fashion-hero.png" &&
    productAvailableStock(product) > 0 &&
    !["İç Giyim", "Aksesuar"].includes(mainCategory)
  );
}

function rankedHomeProducts(products = []) {
  return products
    .filter(usableHomeProduct)
    .sort(
      (a, b) =>
...
function balancedHomeProducts(products = [], limit = 8) {
  const ranked = rankedHomeProducts(products);
  const women = ranked.filter((product) => normalizedGenderSections(product).includes("Kadın"));
  const men = ranked.filter((product) => normalizedGenderSections(product).includes("Erkek"));
  const unisex = ranked.filter((product) => normalizedGenderSections(product).includes("Unisex"));
  const result = [];
  const used = new Set();
  const push = (product) => {
    if (!product || used.has(product.id) || result.length >= limit) return;
    used.add(product.id);
    result.push(product);
  };

  for (let index = 0; result.length < limit && index < Math.max(women.length, men.length, unisex.length); index += 1) {
    push(women[index]);
    push(men[index]);
    if (index % 2 === 0) push(unisex[index]);
  }
  ranked.forEach(push);
...
async function initHome() {
  ...
  try {
    ...
    renderTicker(data.settings);
    initHeroCarousel();
    const products = balancedHomeProducts(data.products, 8);
    const studioProducts = balancedHomeProducts(data.products.slice(8).concat(data.products.slice(0, 8)), 6);
    if (root) {
      root.innerHTML = products.length
...
function renderCategoryTabs(products) {
  ...
  tabs.innerHTML = categories
    .map((category) => {
      ...
      return `<button class="category-tab${active}" type="button" data-category="${escapeHtml(category)}"><span>${escapeHtml(label)}</span></button>`;
    })
    .join("");
}
...
function renderSubcategoryTabs(products) {
  ...
  tabs.innerHTML = subcategories
    .map((subcategory) => {
      ...
      return `<button class="subcategory-tab${active}" type="button" data-subcategory="${escapeHtml(subcategory)}"><span>${escapeHtml(label)}</span></button>`;
    })
    .join("");
}
...
function renderCatalogFilters(products) {
  ...
  const baseProducts = products.filter((product) => productMatchesCategory(product, state.activeCategory, state.activeSubcategory));
  let genderRoot = qs("[data-gender-filter]", panel);

  if (!state.catalogFilters.maxPrice) {
    state.catalogFilters.maxPrice = maxPrice;
  }
  const genderOptions = { compact: true, allowedValues: allowedGenderValuesForCategory(state.activeCategory) };
  state.catalogFilters.gender = normalizeAllowedGender(state.catalogFilters.gender, state.activeCategory);
  if (!genderRoot) {
    panel.insertAdjacentHTML("afterbegin", renderGenderFilterMarkup(state.catalogFilters.gender, baseProducts, genderOptions));
    genderRoot = qs("[data-gender-filter]", panel);
  } else {
    genderRoot.outerHTML = renderGenderFilterMarkup(state.catalogFilters.gender, baseProducts, genderOptions);
  }
  if (sizeSelect) {
...
function renderCatalog(products) {
  ...
  const filtered = products.filter((product) => {
    ...
    const genderMatch = productHasGender(product, filters.gender);
    const { hasSale } = priceMeta(product);
    ...
    return categoryMatch && genderMatch && saleMatch && stockMatch && sizeMatch && colorMatch && priceMatch && (!query || haystack.includes(query));
  });
...
async function initCatalog() {
  ...
  try {
    ...
    state.catalogFilters.gender = normalizeAllowedGender(params.get("gender") || "all", state.activeCategory);
    ...
    qs("[data-catalog-filters]")?.addEventListener("click", (event) => {
      const button = event.target.closest("[data-filter-gender]");
      if (!button) return;
      state.catalogFilters.gender = button.dataset.filterGender || "all";
      state.catalogVisible = state.catalogPageSize;
      renderCatalogFilters(data.products);
      renderCatalog(data.products);
    });
...
async function initCollection() {
  ...
  try {
    ...
    const activeSubcategory = params.get("subcategory")
      ...
      : route.subcategory;
    const activeGender = normalizeAllowedGender(params.get("gender") || "all", activeCategory || "all");
    const baseProducts = activeCategory
      ...
      : data.products;
    const products = collectionProductsForDisplay(
      activeGender === "all" ? baseProducts : baseProducts.filter((product) => productHasGender(product, activeGender)),
      activeCategory,
      activeSubcategory || "all"
    );
    const heroProduct = products[0] || data.products[0];
    const categoryLinks = categoryKeys()
      .map(
        (category) =>
          `<a class="${category === activeCategory && !activeSubcategory ? "is-active" : ""}" href="${genderUrl(categoryUrl(category), activeGender)}">${escapeHtml(category)}</a>`
      )
      .join("");
    const genderLinks = renderGenderFilterMarkup(activeGender, baseProducts, {
      asLinks: true,
      compact: true,
      baseUrl: categoryUrl(activeCategory || "all", activeSubcategory || "all"),
      allowedValues: allowedGenderValuesForCategory(activeCategory || "all")
    });
    const subcategoryLinks = activeCategory
      ? `
        <div class="collection-subtabs">
          <a class="${!activeSubcategory ? "is-active" : ""}" href="${genderUrl(categoryUrl(activeCategory), activeGender)}">Tüm ${escapeHtml(activeCategory)}</a>
          ${CATEGORY_TREE[activeCategory]
            .map(
              (subcategory) =>
                `<a class="${subcategory === activeSubcategory ? "is-active" : ""}" href="${genderUrl(categoryUrl(activeCategory, subcategory), activeGender)}">${escapeHtml(subcategory)}</a>`
            )
            .join("")}
        </div>
      `
    ...
    hero.innerHTML = `
      <div>
        ...
        <div class="collection-tabs">${categoryLinks}<a href="${genderUrl("collection.html", activeGender)}">Tüm kapsül</a></div>
        ${genderLinks}
        ${subcategoryLinks}
      </div>
      ...
      <div class="product-detail__content">
        ...
        <div class="purchase-panel">
          ...
          <div class="product-service-grid">
            ...
            <span>${escapeHtml(cleanDetailText(product.modelInfo, 160) || "Beden bilgisi ürün detaylarına göre güncellenir.")}</span>
        ...
        <div class="product-accordion">
          <details open>
            ...
            <ul class="feature-list">${features || "<li>Ürün özellikleri yakında detaylandırılacaktır.</li>"}</ul>
          </details>
          <details>
            <summary>Materyal ve bakım</summary>
            <div class="care-box">
              <p><strong>Materyal:</strong> ${escapeHtml(productMaterialText(product) || "Ürün materyal bilgisi yakında güncellenecektir.")}</p>
              <p><strong>Bakım:</strong> ${escapeHtml(productCareText(product) || "Etiket talimatına uygun hassas bakım önerilir.")}</p>
              ${product.sku ? `<p><strong>SKU:</strong> ${escapeHtml(product.sku)}</p>` : ""}
            </div>
          </details>
          <details>
            <summary>Detaylar</summary>
            <dl class="spec-list">${specs || "<div><dt>Bilgi</dt><dd>Ürün bilgileri yakında güncellenecektir.</dd></div>"}</dl>
          </details>
        </div>
...
async function localAdminApi(url, options = {}) {
  ...
  if (method === "PUT" && url === "/api/admin/settings") {
    ...
    data.settings.tickerItems = parseTickerItems(body.tickerItems ?? data.settings.tickerItems);
...
function renderSettingsForm() {
  ...
  const fieldValues = {
    ...
    tickerItems: parseTickerItems(settings.tickerItems).join(", "),
...
function settingsFormPayload(form) {
  ...
  return {
    ...
    tickerItems: formData.get("tickerItems"),
...
function initAdminEvents() {
  qs("[data-login-form]")?.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (username === "Efe" && password === "Efe.0107" && looksLikeOfflineApiError(error)) {
        state.admin.mode = "local";
