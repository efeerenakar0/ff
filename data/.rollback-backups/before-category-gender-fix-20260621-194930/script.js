...
const SITE_DATA_CACHE_VERSION = "site-lite-restore-20260621";
...
function productIsInWomenSection(product = {}) {
  const mainCategory = productMainCategory(product);
  const genderSections = normalizedGenderSections(product);
  return mainCategory === "Kadın" || genderSections.includes("Kadın");
}
...
function initPwaShell() {
  ...
  if ("caches" in window) {
    caches.keys().then((keys) => {
      keys
        .filter((key) => key.startsWith("threon-") && key !== "threon-restore-v20260621")
        .forEach((key) => caches.delete(key));
    }).catch(() => {});
  }
  if ("serviceWorker" in navigator && location.hostname !== "") {
    navigator.serviceWorker.register("service-worker.js?v=threon-restore-20260621").catch(() => {});
  }
}
...
function renderCatalog(products) {
  ...
  const filtered = products.filter((product) => {
    ...
    return categoryMatch && saleMatch && stockMatch && sizeMatch && colorMatch && priceMatch && (!query || haystack.includes(query));
  });
...
async function initCollection() {
  ...
  try {
    ...
    const categoryLinks = categoryKeys()
      .map(
        (category) =>
          `<a class="${category === activeCategory && !activeSubcategory ? "is-active" : ""}" href="${categoryUrl(category)}">${escapeHtml(category)}</a>`
      )
      .join("");
    const subcategoryLinks = activeCategory
      ? `
        <div class="collection-subtabs">
          <a class="${!activeSubcategory ? "is-active" : ""}" href="${categoryUrl(activeCategory)}">Tüm ${escapeHtml(activeCategory)}</a>
          ${CATEGORY_TREE[activeCategory]
            .map(
              (subcategory) =>
                `<a class="${subcategory === activeSubcategory ? "is-active" : ""}" href="${categoryUrl(activeCategory, subcategory)}">${escapeHtml(subcategory)}</a>`
            )
            .join("")}
        </div>
      `
