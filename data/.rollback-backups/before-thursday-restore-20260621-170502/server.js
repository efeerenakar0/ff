...
function requireAdmin(req, res) {
  ...
  if (!token || !sessions.has(token)) {
    sendError(res, 401, "Yönetim paneline yeniden giriş yapın.");
    return false;
  }
  return true;
}
...
function saveUploadedImage(body = {}) {
  ...
  if (buffer.length > 24_000_000) {
    throw new Error("Görsel çok büyük. Admin paneli otomatik optimize ettikten sonra tekrar deneyin.");
  }
...
function relatedProductCategory(product = {}) {
  return cleanString(product.category);
}

function relatedProductSubcategory(product = {}) {
  return cleanString(product.subcategory);
}

function relatedProductColorTokens(product = {}) {
  const text = [product.name, product.slug, ...(Array.isArray(product.colors) ? product.colors : [])]
    .filter(Boolean)
    .join(" ")
...
function relatedColorAffinityScore(source = {}, candidate = {}) {
  const sourceColors = new Set(relatedProductColorTokens(source));
  const candidateColors = new Set(relatedProductColorTokens(candidate));
...
function relatedComplementaryScore(source = {}, candidate = {}) {
  const sourceCategory = relatedProductCategory(source);
  const candidateCategory = relatedProductCategory(candidate);
  const sourceSubcategory = relatedProductSubcategory(source);
  const candidateSubcategory = relatedProductSubcategory(candidate);
  let score = 0;
...
function relatedProductScore(source = {}, candidate = {}) {
  if (!candidate || candidate.id === source.id || candidate.status !== "active") return -Infinity;
  const sourceCategory = relatedProductCategory(source);
  const candidateCategory = relatedProductCategory(candidate);
  const sourceSubcategory = relatedProductSubcategory(source);
  const candidateSubcategory = relatedProductSubcategory(candidate);
  const complement = relatedComplementaryScore(source, candidate);
  const colorScore = relatedColorAffinityScore(source, candidate);
  let score = productMerchandisingScore(candidate) + complement + colorScore;
  ...
  if ((Number(candidate.stock) || 0) > 0) score += 28;
...
function relatedProductsFor(source = {}, products = [], limit = 12) {
  return products
    .filter((product) => product.status === "active" && product.id !== source.id)
    .map((product) => ({ product, score: relatedProductScore(source, product) }))
    .filter((item) => Number.isFinite(item.score))
    .sort((a, b) => b.score - a.score || cleanString(a.product.name).localeCompare(cleanString(b.product.name), "tr"))
    .slice(0, limit)
...
function writePublicDataCache(data) {
  try {
    const tempPath = `${PUBLIC_DATA_PATH}.tmp`;
    fs.writeFileSync(tempPath, JSON.stringify(publicData(data)));
    fs.renameSync(tempPath, PUBLIC_DATA_PATH);
...
function readPublicData() {
  ...
  const next = publicData(readData());
  try {
    const tempPath = `${PUBLIC_DATA_PATH}.tmp`;
...
async function handleApi(req, res, pathname) {
  ...
  if (req.method === "GET" && productDetailMatch) {
    ...
    const related = relatedProductsFor(product, data.products, 12);
  ...
  if (req.method === "POST" && reviewMatch) {
    ...
    writeData(data);
    sendJson(res, 201, { review, message: "Yorum onay için admin paneline gönderildi." });
    return;
  }
